...
public class ExprGen{
    ...
    public String printexpr(String class_name, AST.method method, AST.expression expr, List<String> new_formals,
        ...
        case "object": {
            ...
            return visit(temp, class_name, ci, method, new_formals, out);
        }
        case "comp": {
            AST.comp temp = (AST.comp) expr;
            String op = printexpr(class_name, method, temp, new_formals, blocks, out);
            out.println("\t%" + (++mygen.vars) + " = sub nsw i32 1, " + op.substring(4));
            return "i32 %" + mygen.vars;
        }
        case "eq": {
            AST.eq temp = (AST.eq) expr;
            String op1 = printexpr(class_name, method, temp.e1, new_formals, blocks, out);
            String op2 = printexpr(class_name, method, temp.e2, new_formals, blocks, out);
        ...
        case "neg": {
            ...
            String op = printexpr(class_name, method, temp.e1, new_formals, blocks, out);
        ...
        case "assign": {
            ...
            String lvalue = printexpr(class_name, method, temp.e1, new_formals, blocks, out);
            return visit(temp, ci, lvalue, class_name, method, new_formals, out);
        }
        case "block": {
            AST.block temp = (AST.block) expr;
            String return_var = "";
            for (AST.expression e : temp.l1) {
                return_var = printexpr(class_name, method, e, new_formals, blocks, out);
            }
            return return_var;
        }
        case "loop": {
            AST.loop temp = (AST.loop) expr;
            return visit(temp, class_name, method, new_formals, out, blocks);
        }
        case "cond": {
            AST.cond temp = (AST.cond) expr;
            return visit(temp, class_name, method, blocks, new_formals, out);
        }
    ...
    public String visit(AST.new_ expr, PrintWriter out) {
        String type = mygen.gettype(expr.typeid);
    ...
    public String visit(AST.assign expr, ClassIR ci, String lvalue, String class_name, AST.method method,
            List<String> new_formals, PrintWriter out) {
        ...
        String type = mygen.gettype(expr.type);
        String ctype = mygen.gettype(class_name);
        ctype = ctype.substring(0, ctype.length() - 1);
        String ltype = mygen.getvar(lvalue);
        ...
        if (attri == -1) {
            if (new_formals.indexOf(expr.name) == -1) {
                out.println("%" + expr.name + ".addr = alloca " + type + ", align 4");
                new_formals.add(expr.name);
            }
    ...
    public String visit(AST.loop expr, String class_name, AST.method method, List<String> new_formals,
        ...
        String pred = printexpr(class_name, method, expr.predicate, new_formals, blocks, out);
        out.println(
        ...
        String body = printexpr(class_name, method, expr.body, new_formals, blocks, out);
        out.println("\tbr label %loop.cond" + loopcount);
        out.println();
    ...
    public String visit(AST.cond expr, String class_name, AST.method method, List<String> blocks,
        ...
        out.println("if.then" + mygen.ifs + ":");
        blocks.add("if.then" + mygen.ifs);
        String ifbody = printexpr(class_name, method, expr.ifbody, new_formals, blocks, out);
        String ifbodylabel = blocks.get(blocks.size() - 1);
        ifbody = mygen.getvarvalue(ifbody);
        out.println("\tbr label %if.end" + mygen.ifs);
        out.println();
        out.println("if.else" + mygen.ifs + ":");
        blocks.add("if.else" + mygen.ifs);
        String elsebody = printexpr(class_name, method, expr.elsebody, new_formals, blocks, out);
        String elsebodylabel = blocks.get(blocks.size() - 1);
        elsebody = mygen.getvarvalue(elsebody);
        out.println("\tbr label %if.end" + mygen.ifs);
        out.println();
        out.println("if.end" + mygen.ifs + ":");
        blocks.add("if.end" + mygen.ifs);
        out.println("\t%" + (++mygen.vars) + " = phi " + mygen.gettype(expr.type) + " [" + ifbody + ", %" + ifbodylabel + "], ["
                + elsebody + ", %" + elsebodylabel + "]");
        return mygen.gettype(expr.type) + " %" + mygen.vars;
    }

    public String visit(AST.static_dispatch expr, String class_name, AST.method method, List<String> blocks,
            List<String> new_formals, PrintWriter out) {
        String clr = printexpr(class_name, method, expr.caller, new_formals, blocks, out);
        List<String> actuals = new ArrayList<>();
        for (AST.expression e : expr.actuals) {
            String a = printexpr(class_name, method, e, new_formals, blocks, out);
            actuals.add(a);
        }
        mygen.ifs++;
        ...
        ClassIR table2 = mygen.classtable.irTable.get(mygen.reverseParseType(mygen.getvar(clr)));
        while (!mygen.getvar(clr).equals(mygen.gettype(expr.typeid))) {
            String par = mygen.gettype(table2.parent);
            par = par.substring(0, par.length() - 1);
            String ty = mygen.getvar(clr);
            ty = ty.substring(0, ty.length() - 1);
            out.println("\t%" + (++mygen.vars) + " = getelementptr inbounds " + ty + ", " + ty + "* "
                    + mygen.getvarvalue(clr) + ", i32 0, i32 0");
            clr = par + "* %" + mygen.vars;
        ...
        out.println("\t%" + (++mygen.vars) + " = call " + mygen.gettype(expr.type) + " " + funcname + "(" + actstr + ")");
        return mygen.gettype(expr.type) + " %" + mygen.vars;
    }

    public String visit(AST.object expr, String class_name, ClassIR ci, AST.method method, List<String> new_formals,
            PrintWriter out) {
        ...
        if (atr == -1) {
            if (new_formals.indexOf(expr.name) == -1)
                return mygen.gettype(expr.type) + " %" + expr.name;
            else {
                String type = mygen.gettype(expr.type);
        ...
        String typename = mygen.gettype(class_name);
        typename = typename.substring(0, typename.length() - 1);
        out.println("\t%" + (++mygen.vars) + " = getelementptr inbounds " + typename + ", " + typename
                + "* %self, i32 0, i32 " + atr);
        out.println("\t%" + (++mygen.vars) + " = load " + mygen.gettype(expr.type) + ", " + mygen.gettype(expr.type) + "* %"
